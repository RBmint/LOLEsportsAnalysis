...
public class CustomCircle extends View {
    ...
    public CustomCircle(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        outsideColor = a.getColor(R.styleable.CustomCircle_outside_color, ContextCompat.getColor(getContext(), R.color.colorPrimary));
        outsideRadius = a.getDimension(R.styleable.CustomCircle_outside_radius, DimenUtil.dp2px(getContext(), 60.0f));
        insideColor = a.getColor(R.styleable.CustomCircle_inside_color, ContextCompat.getColor(getContext(), R.color.inside_color));
        progressTextColor = a.getColor(R.styleable.CustomCircle_progress_text_color, ContextCompat.getColor(getContext(), R.color.teamBlue));
        progressTextSize = a.getDimension(R.styleable.CustomCircle_progress_text_size, DimenUtil.dp2px(getContext(), 32.0f));
        progressWidth = a.getDimension(R.styleable.CustomCircle_progress_width, DimenUtil.dp2px(getContext(), 10.0f));
        progress = a.getFloat(R.styleable.CustomCircle_progress, 50.0f);
        maxProgress = a.getInt(R.styleable.CustomCircle_max_progress, 100);
        direction = a.getInt(R.styleable.CustomCircle_direction, 0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //第二步:画进度(圆弧)
        paint.setColor(progressTextColor);  //设置进度的颜色
        ...
        canvas.drawArc(oval, 180.0f, 360 * (progress / maxProgress), false, paint);  //根据进度画圆弧
        paint.setColor(outsideColor);
        canvas.drawArc(oval, 180f, -360 * (1 - progress / maxProgress), false, paint);

        //第三步:画圆环内百分比文字
        rect = new Rect();
        paint.setColor(progressTextColor);
        System.out.println("text" + progressTextColor);
    ...
    private String getProgressText() {
        return (float)Math.round((progress / maxProgress)*1000)/10 + "%";
    }
    ...
    private void startAnim(float startProgress) {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                CustomCircle.this.progress = (float) animation.getAnimatedValue();
                postInvalidate();
            }
        });
